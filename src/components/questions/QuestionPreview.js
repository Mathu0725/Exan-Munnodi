    ...
    <div className='space-y-4'>
      <div>
          ...
          <div className='mt-2 mb-3'>
            ...
            {startQuestion && endQuestion && (
              <span className='ml-2 text-sm text-gray-600 dark:text-gray-400'>
                Questions {startQuestion} to {endQuestion} (
                {endQuestion - startQuestion + 1} questions)
              </span>
            )}
          </div>
        )}
        ...
        {typeof questionBody === 'string' && questionBody.length > 0 && (
          <div
            className='prose max-w-none mt-2 text-gray-700 dark:text-gray-300'
        ...
        <div>
          ...
          <ul className='space-y-2'>
            {options.map(opt => (
              <li
                key={opt.id}
                className='p-3 border border-gray-200 dark:border-gray-600 rounded flex items-start gap-2 bg-gray-50 dark:bg-gray-700'
              >
                <span
                  className={`mt-1 inline-block h-2 w-2 rounded-full ${opt.is_correct ? 'bg-green-600' : 'bg-gray-300'}`}
                ></span>
      ...
      {question_type === 'section_based' && startQuestion && endQuestion && (
        <div className='bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800'>
          ...
          <p className='text-xs text-blue-600 dark:text-blue-400 mt-1'>
            Total marks: {marks * (endQuestion - startQuestion + 1)}({marks}{' '}
            marks × {endQuestion - startQuestion + 1} questions)
          </p>
        </div>
      )}
