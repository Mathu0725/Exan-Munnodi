...
export default function SectionBasedQuestion({ onSave, onCancel, initialData = null }) {
    ...
    if (formData.questions.length === 0) {
      ...
      formData.questions.forEach((question, index) => {
        if (!question.title.trim()) {
          newErrors[`question_${index}_title`] = `Question ${index + 1} title is required`;
        }
        if (!question.description.trim()) {
          newErrors[`question_${index}_description`] = `Question ${index + 1} description is required`;
        }
        if (!question.correct_answer) {
          newErrors[`question_${index}_correct_answer`] = `Question ${index + 1} correct answer is required`;
        }
        
        // Check if all options have text
        const emptyOptions = question.options.filter(opt => !opt.text.trim());
        if (emptyOptions.length > 0) {
          newErrors[`question_${index}_options`] = `Question ${index + 1} all options must have text`;
        }
      });
    }
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">
          {initialData ? 'Edit Section-Based Question' : 'Create Section-Based Question'}
        </h2>
        <p className="text-gray-600 dark:text-gray-400">
          Create a question with a description and specify a range of questions (e.g., Questions 1-5)
        </p>
      </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => handleInputChange('startQuestion', parseInt(e.target.value))}
                className={`w-20 px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 ${
                  errors.startQuestion ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                }`}
              />
              ...
              <input
                ...
                onChange={(e) => handleInputChange('endQuestion', parseInt(e.target.value))}
                className={`w-20 px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 ${
                  errors.endQuestion ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                }`}
              />
            ...
            {errors.startQuestion && <p className="text-red-500 text-sm mt-1">{errors.startQuestion}</p>}
            {errors.endQuestion && <p className="text-red-500 text-sm mt-1">{errors.endQuestion}</p>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mb-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md">
            <p className="text-sm text-blue-800 dark:text-blue-200">
              <strong>Instructions:</strong> Write the main paragraph or description that students will read. This will be the basis for the quiz questions below.
            </p>
          </div>
        ...
        <div>
          ...
          <div className="mb-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-md">
            <p className="text-sm text-green-800 dark:text-green-200">
              <strong>Tip:</strong> Upload an image that relates to your paragraph. Students will see both the image and paragraph before answering the quiz questions.
            </p>
          </div>
          <ImageUploader
        ...
        <div>
          ...
          <div className="space-y-6">
            {formData.questions.map((question, index) => (
              <div key={question.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-6 bg-white dark:bg-gray-800">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 ${
                      errors[`question_${index}_title`] ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
                    placeholder="Enter question title"
                  />
                ...
                <div className="mb-4">
                  ...
                  {errors[`question_${index}_description`] && (
                    <p className="text-red-500 text-sm mt-1">{errors[`question_${index}_description`]}</p>
                  )}
                </div>
                ...
                <div className="mb-4">
                  ...
                  <ImageUploader
                    onImageUpload={(url) => handleQuestionImageUpload(question.id, url)}
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {question.options.map((option, optionIndex) => (
                      <div key={option.id} className="flex items-center space-x-4 p-3 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700">
                  ...
                  {errors[`question_${index}_correct_answer`] && (
                    <p className="text-red-500 text-sm mt-1">{errors[`question_${index}_correct_answer`]}</p>
                  )}
                </div>
              </div>
            ))}
