  ...
  useFrame(state => {
    if (meshRef.current) {
      meshRef.current.rotation.y =
        rotation[1] + Math.sin(state.clock.elapsedTime * speed) * 0.1;
...
function Book({ position, color, speed = 1 }) {
  ...
  useFrame(state => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y =
        position[1] + Math.sin(state.clock.elapsedTime * speed * 0.7) * 0.3;
    }
  });
...
function Laptop({ position, color, speed = 1 }) {
  ...
  useFrame(state => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y =
        position[1] + Math.sin(state.clock.elapsedTime * speed * 0.6) * 0.25;
    }
  });
...
function Chart({ position, color, speed = 1 }) {
  ...
  useFrame(state => {
    if (meshRef.current) {
      meshRef.current.rotation.z += 0.01 * speed;
      meshRef.current.position.y =
        position[1] + Math.sin(state.clock.elapsedTime * speed * 0.8) * 0.35;
    }
  });
...
function DataParticle({ position, color, speed = 1 }) {
  ...
  useFrame(state => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y =
        position[1] + Math.cos(state.clock.elapsedTime * speed) * 1.5;
      meshRef.current.position.z =
        position[2] + Math.sin(state.clock.elapsedTime * speed * 0.5) * 1;
    }
  });
