...
export default function PageWrapper({ children, title }) {
  ...
  const isProfileRoute = useMemo(() => pathname.startsWith('/profile'), [pathname]);
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Please <Link href="/login" className="text-indigo-600">sign in</Link> to continue.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md text-center space-y-3">
          ...
          <p className="text-sm text-gray-600">
            Your account is currently <strong>{user.status}</strong>. Administrators must approve your
            registration before you can access the system.
          </p>
          <p className="text-sm text-gray-500">
            If this takes longer than expected, please contact the administrator.
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 theme-transition">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 flex lg:hidden">
          <div className="fixed inset-0 bg-black/40" onClick={() => setSidebarOpen(false)} aria-hidden="true"></div>
