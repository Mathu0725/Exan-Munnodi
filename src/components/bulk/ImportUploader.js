...
const normalizeRowForImport = (row, subjectNameToIdMap, categoryNameToIdMap, defaults) => {
...
const validateRow = (row, index, defaults) => {
  ...
  if (!normalizedDifficulty || !['1','2','3','4','5'].includes(normalizedDifficulty)) errors.push('Difficulty must be between 1 and 5');

  const rawMarks = row.marks ?? defaults?.marks ?? '';
  const normalizedMarks = String(rawMarks).trim();
  if (!normalizedMarks || isNaN(parseFloat(normalizedMarks))) errors.push('Valid Marks are required');

  const rawNegativeMarks = row.negative_marks ?? defaults?.negativeMarks ?? '0';
  const normalizedNegativeMarks = String(rawNegativeMarks).trim() === '' ? '0' : String(rawNegativeMarks).trim();
  if (isNaN(parseFloat(normalizedNegativeMarks))) errors.push('Negative marks must be a number');
  
  const options = [row.option1, row.option2, row.option3, row.option4, row.option5].filter((opt) => opt !== undefined && opt !== null && String(opt).trim() !== '');
  if (options.length < 2) errors.push('At least 2 options are required');
  const correctAnswerSource = row.correct_answer ?? '';
  if (!correctAnswerSource || isNaN(parseInt(correctAnswerSource)) || parseInt(correctAnswerSource) < 1 || parseInt(correctAnswerSource) > options.length) {
  ...
  return {
    ...
    data: {
      ...
      options: options.map((opt, i) => ({ text: opt, is_correct: (i + 1) === parseInt(correctAnswerSource) })),
...
export default function ImportUploader() {
  ...
  const subjectOptions = useMemo(() => subjectHierarchyData?.data || [], [subjectHierarchyData]);
  const categoryOptions = useMemo(() => categoriesData?.data || [], [categoriesData]);

  const selectedSubjectName = useMemo(() => {
    if (!subjectId) return '';
    const match = subjectOptions.find((subject) => String(subject.id) === String(subjectId));
    return match?.name || '';
  ...
  const defaultHints = useMemo(() => {
    ...
    if (defaultMarks) hints.push(`${defaultMarks} mark${Number(defaultMarks) === 1 ? '' : 's'}`);
    ...
    <div className="bg-white p-6 rounded-lg shadow space-y-6">
      ...
      <h3 className="text-lg font-medium">Import Questions from CSV</h3>
              <p className="text-sm text-gray-500 mt-1">
                First select subject and sub-subject, then upload CSV with just questions and answers.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-lg border border-blue-200">
            <div className="flex items-center mb-4">
              ...
              <h4 className="font-semibold text-blue-900 text-lg">Choose Subject & Category</h4>
            </div>
            <p className="text-blue-700 text-sm mb-4">Select where these questions will be categorized</p>
              ...
              <div className="space-y-2">
                ...
                {subjectId && (
                  <div className="flex items-center text-green-600 text-sm">
                    ...
                    Selected: {subjectOptions.find(s => s.id == subjectId)?.name}
                  </div>
                )}
              </div>
            ...
            <div className="mt-4">
              ...
              {categoryId && (
                <div className="flex items-center text-green-600 text-sm mt-1">
                  ...
                  Selected: {categoryOptions.find(c => c.id == categoryId)?.name}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg border border-green-200">
            <div className="flex items-center mb-4">
              ...
              <h4 className="font-semibold text-green-900 text-lg">Upload Your Questions</h4>
            </div>
            ...
            {!subjectId || !categoryId ? (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <p className="text-yellow-800 font-medium">Please complete Step 1 first</p>
                </div>
                <p className="text-yellow-700 text-sm mt-1">Select subject and category before uploading CSV</p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="bg-white border-2 border-dashed border-green-300 rounded-lg p-8 text-center hover:border-green-400 transition-colors">
                  ...
                  <h5 className="text-lg font-semibold text-gray-900 mb-2">Upload CSV File</h5>
                  <p className="text-gray-600 mb-4">
                    Upload your questions CSV file. Make sure it has these columns:
                  </p>
                  <div className="bg-gray-50 rounded-lg p-3 mb-4">
                    <code className="text-sm text-gray-700">
                      question, option1, option2, option3, option4, option5, correct_answer, difficulty
                    </code>
                  </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-blue-800 font-medium text-sm">Questions will be assigned to:</p>
                      <p className="text-blue-700 text-sm">
                        <strong>{subjectOptions.find(s => s.id == subjectId)?.name}</strong>
                        {subSubjectId && ` > ${subSubjectsData?.data.find(s => s.id == subSubjectId)?.name}`}
                        <br />
                        Category: <strong>{categoryOptions.find(c => c.id == categoryId)?.name}</strong>
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {isProcessing && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 text-center">
              ...
              <h5 className="text-lg font-semibold text-purple-900 mb-2">Processing Your Questions</h5>
              <p className="text-purple-700">Please wait while we validate your CSV file...</p>
            </div>
          )}
        </>
      ) : (
        <>
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium">Configure Question Settings</h3>
              <p className="text-sm text-gray-500 mt-1">
                CSV uploaded successfully! Now select the subject, category, and other settings for all {validationResult?.valid.length || 0} questions.
              </p>
            </div>
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <select value={subjectId} onChange={(e) => setSubjectId(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md">
            ...
            <div className="space-y-1">
              <label className="text-xs uppercase tracking-wide text-gray-500">Sub-subject</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="space-y-1">
          <label className="text-xs uppercase tracking-wide text-gray-500">Default Difficulty (1-5)</label>
          <select value={defaultDifficulty} onChange={(e) => setDefaultDifficulty(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md">
        ...
        <div className="space-y-1">
          <label className="text-xs uppercase tracking-wide text-gray-500">Default Marks</label>
          <input
            type="number"
            min="0"
            step="0.5"
        ...
        <div className="space-y-1">
          <label className="text-xs uppercase tracking-wide text-gray-500">Default Negative Marks</label>
          <input
            type="number"
            min="0"
            step="0.5"
          ...
          {validationResult && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-lg border border-green-200">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-green-700 mt-1">
                    {validationResult.valid.length} questions are ready to be imported
                  </p>
                </div>
              ...
              {validationResult.valid.length > 0 && (
                <div className="mb-6">
                  <h5 className="font-semibold text-gray-900 mb-3">Preview Questions</h5>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Correct Answer</th>
                          </tr>
                        </thead>
                            ...
                            <tr key={i} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm">
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                  {row.difficulty || row.data?.difficulty || 'N/A'}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-sm text-gray-900">
                                Option {row.correct_answer || row.data?.correct_answer || 'N/A'}
                              </td>
                            </tr>
                          ))}
                            ...
                            <tr className="bg-gray-50">
                              <td colSpan="3" className="px-4 py-3 text-center text-sm text-gray-500">
                                <div className="flex items-center justify-center">
                                  ...
                                  ... and {validationResult.valid.length - 5} more questions
                                </div>
                              </td>
                            </tr>
                          )}
              ...
              <div className="flex justify-between items-center">
                ...
                <button 
                  onClick={handleSave} 
                  disabled={!subjectId || !categoryId || validationResult.valid.length === 0 || mutation.isPending} 
              ...
              {validationResult.invalid.length > 0 && (
                <div className="mt-6">
                  <h4 className="font-semibold text-lg mb-2">Errors Found ({validationResult.invalid.length})</h4>
                  <div className="overflow-x-auto border rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-medium">Row</th>
                          <th className="px-4 py-2 text-left text-xs font-medium">Question</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {validationResult.invalid.map((row, i) => (
                          <tr key={i} className="bg-red-50">
                            <td className="px-4 py-2">{row.originalIndex || i + 1}</td>
                            <td className="px-4 py-2">{row.data?.title || row.title || 'N/A'}</td>
      ...
      {!validationResult && (
        <>
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center space-y-3">
            <p className="text-sm text-gray-600">Upload the template CSV. Leave subject/category columns blank if you want the defaults above to apply.</p>
            <input id="csv-upload" type="file" accept=".csv" onChange={handleFileChange} className="w-full max-w-xs mx-auto" />
        ...
        <div>
          <div className="p-4 bg-gray-50 rounded-lg space-y-2">
            ...
            <p className="text-sm text-gray-500">
              Need to fix subject, category, difficulty, or marks? Update the CSV or adjust the defaults above and re-validate.{' '}
              {defaultHints && <span className="text-gray-600">Current defaults: {defaultHints}.</span>}
            </p>
          </div>
