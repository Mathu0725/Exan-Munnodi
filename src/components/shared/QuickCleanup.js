...
export default function QuickCleanup() {
  ...
  const handleDeleteAll = async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${orphanedCount} orphaned questions? This action cannot be undone.`
      )
    ) {
      const questions = questionsData?.data || [];
      const subjects = subjectsData?.data || [];
    ...
    <div className='bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4'>
      ...
      {orphanedCount > 0 && (
        <div className='mt-3 text-xs text-yellow-700 dark:text-yellow-300'>
          ⚠️ Found {orphanedCount} questions with missing categories or
          subjects.
          <a
