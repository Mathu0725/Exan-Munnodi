...
export default function QuestionCleanup() {
  ...
  const scanForOrphanedQuestions = async () => {
    ...
    for (const question of questions) {
      ...
      if (
        selectedFilters.noSubject &&
        (!question.subject_id ||
          !subjects.find(s => s.id === question.subject_id))
      ) {
      ...
      if (
        selectedFilters.noSubSubject &&
        question.sub_subject_id &&
        !subjects.find(s =>
          s.subsubjects?.find(ss => ss.id === question.sub_subject_id)
        )
      ) {
  ...
  const handleDeleteSelected = async questionIds => {
    ...
    const protectedQuestions = orphanedQuestions.filter(
      q => questionIds.includes(q.id) && q.isProtected
    );
    const deletableIds = questionIds.filter(
      id => !orphanedQuestions.find(q => q.id === id && q.isProtected)
    );

    if (protectedQuestions.length > 0) {
      alert(
        `Cannot delete ${protectedQuestions.length} question(s) because they are used in live exams:\n${protectedQuestions.map(q => `- ${q.title} (ID: ${q.id})`).join('\n')}`
      );
    }
    ...
    if (
      window.confirm(
        `Are you sure you want to delete ${deletableIds.length} questions? This action cannot be undone.`
      )
    ) {
      await deleteQuestionsMutation.mutateAsync(deletableIds);
      setOrphanedQuestions(prev =>
        prev.filter(q => !deletableIds.includes(q.id))
      );
    }
  };
                ...
                >
                  <div className='flex-1'>
                    ...
                    <div className='text-xs text-gray-400 dark:text-gray-500 mt-1'>
                      ...
                      {question.category_id || 'None'} | Subject:{' '}
                  ...
                  <button
                    ...
                    title={
                      isProtected
                        ? 'Cannot delete: Used in live exam(s)'
                        : 'Delete question'
                    }
                  >
