...
export default function Step4Preview({ data, onBack, onPublish }) {
  const { title, description, exam_type_id, start_at, end_at, questions = [], config = {} } = data || {};
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
          ...
          <button onClick={() => onPublish('published')} className="px-4 py-2 bg-indigo-600 text-white rounded-md">Publish</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <div>
            ...
            <div className="text-gray-700">{config?.access_password ? '••••••' : '-'}</div>
          </div>
          <div>
        ...
        <div className="space-y-2">
          ...
          <pre className="text-xs bg-gray-50 p-3 rounded border overflow-auto max-h-64">{JSON.stringify(config, null, 2)}</pre>
        </div>
      </div>
