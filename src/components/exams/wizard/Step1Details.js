...
export default function Step1Details({ data, onNext }) {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 mt-8">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">Assign to Student Groups</label>
        ...
        {selectedGroups.length > 0 && (
          <p className="mt-2 text-sm text-gray-600">
            📢 {selectedGroups.length} group(s) selected - Students will receive notifications
          </p>
        )}
      </div>
