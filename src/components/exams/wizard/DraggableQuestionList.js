...
export default function DraggableQuestionList({ questions, setQuestions, renderActions, searchTerm, isQuestionMatching }) {
  ...
  const visibleQuestions = searchTerm ? questions.filter(isQuestionMatching) : questions;
    ...
    <div className="mt-4 space-y-2 p-4 border rounded-md bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 min-h-[200px] theme-transition">
      {Object.keys(groupedQuestions).length > 0 ? (
        Object.entries(groupedQuestions).map(([groupId, group]) => {
          const isMatching = isQuestionMatching ? isQuestionMatching(group.parent) : true;
          ...
          if (group.children.length > 0 && !group.isSectionConfig) {
            return (
              <div key={groupId} className="border border-blue-200 dark:border-blue-700 rounded-lg bg-blue-50 dark:bg-blue-900/20 p-3">
                {/* Section Header */}
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="font-medium text-blue-800 dark:text-blue-200">
                        {group.parent.title.replace(' - Question ' + group.parent.questionNumber, '')}
                      </div>
                      <div className="text-sm text-blue-600 dark:text-blue-400">
                        Section-based Question ({group.children.length} questions)
                      </div>
                    </div>
                  </div>
                  <div>
                    {renderActions && renderActions(group.parent, questions.findIndex(q => q.id === group.parent.id))}
                  </div>
                </div>
                ...
                <div className="ml-4 space-y-1">
                  {group.children
                    ...
                    <div key={childQuestion.id} className="flex items-center p-2 bg-white dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600">
                      <span className="text-gray-500 mr-3 text-sm">Q{childQuestion.questionNumber}</span>
                      <div className="flex-1">
                        ...
                        {childQuestion.body && (
                          <div className="text-xs text-gray-600 dark:text-gray-300 mt-1" style={{
                            ...
                            {childQuestion.body.replace(/<[^>]*>/g, '').substring(0, 100)}...
                          </div>
                        )}
                      </div>
                    </div>
          ...
          if (group.isSectionConfig) {
            ...
            return (
              <div key={groupId} className="border border-green-200 dark:border-green-700 rounded-lg bg-green-50 dark:bg-green-900/20 p-3">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-green-600 dark:text-green-400">
                        Section Configuration: {config.selectedCount} of {config.totalQuestions} questions selected
                      </div>
                      <div className="text-xs text-green-500 dark:text-green-300 mt-1">
                        Questions {config.questionRange.start}-{config.questionRange.end} • Random selection for each student
                      </div>
                    </div>
                  </div>
                  <div>
                ...
                <div className="ml-4 p-2 bg-white dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600">
                  <div className="text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium text-green-600">{config.selectedCount}</span>
                    </div>
                    <div className="flex justify-between items-center">
          ...
          return (
            <div
              key={group.parent.id}
              draggable
              onDragStart={(e) => handleDragStart(e, questions.findIndex(q => q.id === group.parent.id))}
              onDragEnter={(e) => handleDragEnter(e, questions.findIndex(q => q.id === group.parent.id))}
              onDragEnd={handleDragEnd}
              onDragOver={(e) => e.preventDefault()}
              className={`flex justify-between items-center p-2 border rounded-md cursor-move transition-all ${
                dragging ? getStyles(questions.findIndex(q => q.id === group.parent.id)) : 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600'
            ...
            <div className="flex items-center flex-1">
              ...
              <div className="flex-1">
                ...
                {group.parent.body && (
                  <div className="text-sm text-gray-600 dark:text-gray-300 mt-1 overflow-hidden tamil-body" style={{
                    display: '-webkit-box',
                    WebkitLineClamp: 2,
                    WebkitBoxOrient: 'vertical'
                  }}>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-center">No questions selected yet.</p>
      )}
      
      {searchTerm && Object.keys(groupedQuestions).length === 0 && questions.length > 0 && (
