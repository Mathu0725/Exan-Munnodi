...
const LatexQuillViewer = dynamic(() => import('@/components/questions/LatexQuillEditor'), { ssr: false });
        ...
        <header className="border-b pb-4 mb-6">
          ...
          {isAdmin && (onEditDetails || onEditQuestions || onEditConfig) && (
            <div className="mt-4 flex flex-wrap gap-3 text-sm">
              {onEditDetails && (
                <button onClick={onEditDetails} className="px-3 py-1 border border-indigo-500 text-indigo-600 rounded-md hover:bg-indigo-50">Edit Exam Details</button>
              )}
              {onEditQuestions && (
                <button onClick={onEditQuestions} className="px-3 py-1 border border-indigo-500 text-indigo-600 rounded-md hover:bg-indigo-50">Edit Questions</button>
              )}
              {onEditConfig && (
                <button onClick={onEditConfig} className="px-3 py-1 border border-indigo-500 text-indigo-600 rounded-md hover:bg-indigo-50">Edit Configuration</button>
              )}
            </div>
          )}
        </header>
                    ...
                    {(q.options || []).map((opt, optIndex) => {
                      ...
                      return (
                        <div key={opt.id || optIndex} className={`flex items-center p-2 border rounded-md ${highlightClasses}`}>
                          ...
                          <label htmlFor={`q-${index}-opt-${optIndex}`} className="flex-1">{opt.text || opt.label || ''}</label>
                          {isAdmin && isCorrect && (
