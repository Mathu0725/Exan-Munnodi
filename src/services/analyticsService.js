...
export const analyticsService = {
  async getDashboardStats() {
    try {
      ...
      const [questionsRes, examsRes, subjectsRes, usersRes] =
      ...
      const questions =
        questionsRes.status === 'fulfilled'
          ? questionsRes.value?.meta?.total || 0
          : 0;
      const exams =
        examsRes.status === 'fulfilled' ? examsRes.value?.meta?.total || 0 : 0;
      const subjects =
        subjectsRes.status === 'fulfilled'
          ? subjectsRes.value?.meta?.total || 0
          : 0;
      const users =
          ...
          recentActivity: [
            {
              ...
              action: 'Created exam "Weekly GK Test"',
            },
            {
              id: 2,
              user: 'editor@example.com',
              action: 'Added 5 new questions to "Mathematics"',
            },
          ],
        },
      };
