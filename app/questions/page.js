...
export default function QuestionsPage() {
  ...
  const closeHistory = () =>
    setHistory({ isOpen: false, versions: [], title: '' });

  const handleDeleteSelected = () => {
    if (
      window.confirm(
        `Are you sure you want to delete ${selected.length} selected questions?`
      )
    ) {
  ...
  const handleBulkStatusUpdate = status => {
    if (
      window.confirm(`Update ${selected.length} questions to ${status} status?`)
    ) {
  ...
  const handleCleanupDuplicates = () => {
    if (
      window.confirm(
        'This will scan all questions and remove duplicates. Continue?'
      )
    ) {
  ...
  return (
    <PageWrapper title='Question Bank'>
          ...
          <table className='min-w-full divide-y divide-gray-200'>
            <thead className='bg-gray-50'>
              <tr>
                ...
                <th className='px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase w-80'>
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className='divide-y divide-gray-200'>
              {questions.map(question => (
                <tr
                  key={question.id}
                  className={
                    selected.includes(question.id) ? 'bg-indigo-50' : ''
                  }
                >
                  <td className='p-4'>
                    <input
                  ...
                  <td className='px-6 py-4 text-right'>
                    <div className='flex flex-wrap gap-2 justify-end'>
                      ...
                      {user?.role !== 'Viewer' ? (
                        ...
                        <span className='px-3 py-1.5 text-xs text-gray-400 bg-gray-50 rounded-md'>
                          Read-only
                        </span>
                      )}
                    </div>
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-center text-sm text-gray-700'>
                  <span>
                    ...
                    {Math.min(meta.currentPage * meta.pageSize, meta.total)} of{' '}
      ...
      <Modal
        ...
        title={`Version History - ${history.title}`}
      >
