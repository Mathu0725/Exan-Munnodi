...
export default function SubjectsPage() {
  ...
  const [subSubjectModal, setSubSubjectModal] = useState({
    isOpen: false,
    subSubject: null,
    subjectId: null,
  });
  ...
  const updateSubjectMutation = useMutation({
    mutationFn: vars => subjectService.updateSubject(vars.id, vars.data),
    onSuccess: () => {
      invalidateQueries();
      closeSubjectModal();
    },
  ...
  const createSubSubjectMutation = useMutation({
    mutationFn: subjectService.createSubSubject,
    onSuccess: () => {
      invalidateQueries();
      closeSubSubjectModal();
    },
  });
  const updateSubSubjectMutation = useMutation({
    mutationFn: vars => subjectService.updateSubSubject(vars.id, vars.data),
    onSuccess: () => {
      invalidateQueries();
      closeSubSubjectModal();
    },
  });
  const deleteSubSubjectMutation = useMutation({
    mutationFn: subjectService.deleteSubSubject,
    onSuccess: invalidateQueries,
  });
  ...
  const openSubSubjectModal = (subSubject = null, subjectId) =>
    setSubSubjectModal({ isOpen: true, subSubject, subjectId });
  const closeSubSubjectModal = () =>
    setSubSubjectModal({ isOpen: false, subSubject: null, subjectId: null });
  ...
  const handleSubSubjectSubmit = formData => {
    if (subSubjectModal.subSubject) {
      updateSubSubjectMutation.mutate({
        id: subSubjectModal.subSubject.id,
        data: formData,
      });
    } else {
  ...
  return (
    <PageWrapper title='Subjects & Sub-subjects'>
            ...
            <div key={subject.id} className='bg-white shadow rounded-lg'>
              <div className='p-4 flex justify-between items-center border-b'>
                <div>
                  ...
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${subject.active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                  >
              ...
              <div className='p-4'>
                <div className='flex justify-between items-center mb-2'>
                  ...
                  <button
                    onClick={() => openSubSubjectModal(null, subject.id)}
                    ...
                    {subject.subsubjects.map(ss => (
                      ...
                      >
                        ...
                        <div className='text-sm'>
                          <button
                            onClick={() => openSubSubjectModal(ss, subject.id)}
                            className='text-indigo-600 hover:text-indigo-900 mr-4'
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => handleDelete('sub-subject', ss.id)}
                            className='text-red-600 hover:text-red-900'
                          >
                            Delete
                          </button>
      ...
      <Modal
        ...
        title={subjectModal.subject ? 'Edit Subject' : 'Add Subject'}
      >
        <SubjectForm
      ...
      <Modal
        isOpen={subSubjectModal.isOpen}
        onClose={closeSubSubjectModal}
        title={
          subSubjectModal.subSubject ? 'Edit Sub-subject' : 'Add Sub-subject'
        }
      >
        <SubSubjectForm
          subSubject={subSubjectModal.subSubject}
          subjectId={subSubjectModal.subjectId}
          onSubmit={handleSubSubjectSubmit}
          onCancel={closeSubSubjectModal}
        />
      </Modal>
    </PageWrapper>
  );
