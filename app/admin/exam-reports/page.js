...
import { FaDownload, FaFilter, FaChartBar, FaUsers, FaGraduationCap, FaCalendarAlt } from 'react-icons/fa';
...
const ExamResultRow = ({ result }) => {
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${getScoreColor(percentage)}`}>
...
export default function ExamReportsPage() {
  ...
  const averageScore = totalResults > 0 ? 
    results.reduce((sum, r) => sum + (r.score / r.totalMarks) * 100, 0) / totalResults : 0;
  const passRate = totalResults > 0 ? 
    (results.filter(r => (r.score / r.totalMarks) * 100 >= 60).length / totalResults) * 100 : 0;
  ...
  return (
    <PageWrapper title="Exam Reports">
      <div className="space-y-6">
        ...
        {/* Filters */}
        <ReportFilters filters={filters} onFilterChange={handleFilterChange} exams={exams} onExport={handleExport} />
