...
export default function ProfilePage() {
  ...
  const pendingRequest = useMemo(() => updateRequests.find((req) => req.status === 'Pending'), [updateRequests]);
  ...
  if (!user) {
    return (
      <div className="p-6">
        ...
        <p className="text-sm text-gray-600 mt-2">You must be logged in to view your profile.</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">
          View your account details and request updates. Changes must be approved by an administrator.
        </p>
      </div>

      {error && (
        <div className="rounded-md border border-red-200 bg-red-50 text-red-600 px-4 py-3 text-sm">{error}</div>
      )}

      {success && (
        <div className="rounded-md border border-green-200 bg-green-50 text-green-700 px-4 py-3 text-sm">{success}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex flex-col items-center space-y-4 p-6 bg-gray-50 rounded-lg">
          ...
          <PhotoUpload
            ...
            onPhotoChange={(avatarUrl) => setFormState(prev => ({ ...prev, avatarUrl }))}
        ...
        <div className="flex items-center justify-between border-t border-gray-200 pt-4">
          {pendingRequest ? (
            <p className="text-sm text-gray-600">
              You have a pending update request. Please wait for an administrator to review it.
            </p>
          ) : (
            <p className="text-sm text-gray-500">Changes require administrator approval before they take effect.</p>
          )}
            ...
            {updateRequests.map((request) => (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-medium text-gray-800">Request #{request.id}</div>
                  <span
